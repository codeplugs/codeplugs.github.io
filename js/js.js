...
window.onerror = function (msg, url, line) {
  document.getElementById("debug").innerHTML = "❌ JS ERROR<br>" + msg + "<br>Line: " + line;
};
...
function autoLogin() {
  auth.signInAnonymously().then(function (res) {
    loginStatus.innerHTML = "UID: " + res.user.uid;
    startDashboard();
  })["catch"](function (err) {
    document.getElementById("debug").innerHTML = "❌ AUTH ERROR: " + err.message;
  });
}
auth.onAuthStateChanged(function (user) {
  if (user) {
    loginStatus.innerHTML = "UID: " + user.uid;
    startDashboard();
  } else {
    autoLogin();
  }
...
function startDashboard() {
  ...
  db.ref("cpterminal_logs").once("value").then(function (snap) {
    console.log("PROBE:", snap.val());
  })["catch"](function (err) {
    console.log("PROBE ERROR:", err.message);
  });
  db.ref("cpterminal_logs").on("value", function (snapshot) {
    var debug = document.getElementById("debug");
    console.log("RAW SNAP:", snapshot.val());
    if (!snapshot.exists()) {
      debug.innerHTML = "❌ NO DATA / RULES BLOCK / WRONG PATH<br>" + "snapshot = null";
      return;
    }
    debug.innerHTML = "✅ DATA OK<br>devices: " + snapshot.numChildren();

    //debug.innerHTML = "✅ DEVICES: " + snapshot.numChildren();

    devicesDiv.innerHTML = "";
    snapshot.forEach(function (deviceSnap) {
      var data = deviceSnap.val();
      var uid = deviceSnap.key;
      var collapseId = "dev_" + uid;
      var deviceName = data.model || data.brand || uid;
      var header = document.createElement("div");
      header.className = "device-header";
      header.setAttribute("data-toggle", "collapse");
      header.setAttribute("data-target", "#" + collapseId);
      header.innerHTML = "📱 " + deviceName;
      var body = document.createElement("div");
      body.className = "collapse";
      body.id = collapseId;
      var table = document.createElement("table");
      table.className = "table table-dark table-sm";
      table.innerHTML = "<thead><tr><th>Time</th><th>Type</th><th>Message</th></tr></thead>" + "<tbody id='logs_" + uid + "'></tbody>";
...
function loadLogs(uid) {
  var tbody = document.getElementById("logs_" + uid);
  db.ref("cpterminal_logs/" + uid + "/logs").limitToLast(200).on("value", function (snapshot) {
    var rows = [];
    snapshot.forEach(function (logSnap) {
    ...
    for (var i = 0; i < rows.length; i++) {
      var log = rows[i];
      if (search && log.text.toLowerCase().indexOf(search) === -1) {
        continue;
      }
      var typeClass = "";
      if (log.type === "INFO") typeClass = "table-info";else if (log.type === "ERROR") typeClass = "table-danger";else if (log.type === "WARN") typeClass = "table-warning";else if (log.type === "DEBUG") typeClass = "table-secondary";else typeClass = "table-dark";
      var tr = document.createElement("tr");
      tr.className = typeClass;
      tr.innerHTML = "<td>" + log.type + "</td>" + "<td>" + log.clock + "</td>" + "<td>" + log.text + "</td>";
