...
db.ref("cpterminal_logs").once("value")
.then(function(snap){
...
.catch(function(err){
    ...
    if (!snapshot.exists()) {
        debug.innerHTML =
            "❌ NO DATA / RULES BLOCK / WRONG PATH<br>" +
            "snapshot = null";
        return;
    }

    debug.innerHTML =
        "✅ DATA OK<br>devices: " + snapshot.numChildren();
...
if (log.type === "INFO") typeClass = "table-info";
else if (log.type === "ERROR") typeClass = "table-danger";
else if (log.type === "WARN") typeClass = "table-warning";
else if (log.type === "DEBUG") typeClass = "table-secondary";
else typeClass = "table-dark";
